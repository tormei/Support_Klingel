...
RTC_DATA_ATTR double * _vs = (double*)malloc(101*sizeof(double)); //LiPo-Voltage-Array initialisieren
...
{
    int readValue = _analogRead(ADC_PIN);
...
double _analogReadToVolts(int readValue){
  double volts; 
  volts = readValue * CONV_FACTOR / 1000;
  return volts;
}

double getBatteryVolts(){
    int readValue = analogRead(ADC_PIN);
